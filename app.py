...
STAKE_PERCENTAGES = {1: 0.5, 2: 1.0, 3: 2.0, 4: 3.5, 5: 5.0}
...
def update_match_result_in_sheets(match_str: str, actual_score: str):
    try:
        ...
        result = (
            service.spreadsheets()
            .values()
            .get(spreadsheetId=spreadsheet_id, range="PREDICTIONS!A:W")
            .execute()
        )
...
def save_historical_match(historical_match: Dict) -> bool:
    ...
    try:
        ...
        if not sheet_id:
            st.error(
                "❌ Keine Google Sheets ID gefunden (sheet_id / sheet_id_v48 / sheet_id_v47)"
            )
            return False
                    ...
                    if len(row) > 16 and row[16] == "COMPLETED":
                        ...
                        try:
                            ...
                            match_data = {
                                ...
                                "home_correction": (
                                    float(home_goals) / predicted_mu_home
                                    if predicted_mu_home > 0
                                    else 1.0
                                ),
                                "away_correction": (
                                    float(away_goals) / predicted_mu_away
                                    if predicted_mu_away > 0
                                    else 1.0
                                ),
                                "actual_score": actual_score,
                            }
    ...
    try:
        ...
        try:
            ...
            headers = [
                ...
                "Subs_Home",
                "Subs_Away",
                "Shot_Efficiency_Home",
                "Shot_Efficiency_Away",
                "Corner_Frequency",
                "Foul_Frequency",
                "Cards_Total",
                "Subs_Total",
        ...
        shot_efficiency_home = (
            (extended_data.shots_on_target_home / extended_data.shots_home * 100)
            if extended_data.shots_home > 0
            else 0
        )
        shot_efficiency_away = (
            (extended_data.shots_on_target_away / extended_data.shots_away * 100)
        ...
        foul_frequency = (extended_data.fouls_home + extended_data.fouls_away) / 90

        cards_total = (
            extended_data.yellow_cards_home
            + extended_data.yellow_cards_away
            + extended_data.red_cards_home
        ...
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        match_str = (
            match_info.get("match", "Unknown Match")
            if match_info
            else extended_data.match_id
        )
    ...
    try:
        ...
        try:
            extended_result = (
                service.spreadsheets()
                .values()
                .get(spreadsheetId=sheet_id, range="EXTENDED_DATA!B:B")
                .execute()
            )
            extended_matches = [
                row[0] for row in extended_result.get("values", []) if row
            ]
        except:
            extended_matches = []
...
def create_extended_features(extended_data: ExtendedMatchData, match_info: Dict):
    features = {}
    ...
    if selected_match_info:
        ...
        with st.sidebar.expander("📝 Erweiterte Daten eingeben", expanded=False):
            ...
            with st.form(key="extended_data_form"):
                ...
                st.caption(f"Heim: {possession_home}% | Auswärts: {possession_away}%")
...
def show_ml_training_ui():
    ...
    with col3:
        if ml_model.is_trained and ml_model.last_trained:
            st.metric("Zuletzt trainiert", ml_model.last_trained.strftime("%d.%m.%Y"))
        else:
            ...
            if st.button(
                ...
                with st.spinner("Training ML-Modell mit historischen Daten..."):
                    ...
                    if result["success"]:
                        ...
                        st.error(f"❌ Training fehlgeschlagen: {result['message']}")
...
def display_results(result):
    ...
    with viz_tab1:
        ...
        fig_mu.update_layout(
            title="Smart μ-Werte Vergleich", yaxis_title="Erwartete Tore", height=300
        )
...
def show_sidebar():
    with st.sidebar:
        ...
        st.caption(f"Max. Einsatz: {profile_info['max_stake_percent']}% der Bankroll")
            ...
            if "tracking" not in st.secrets:
                ...
                if not sheet_id:
                    ...
                    if "sheet_id_v48" in st.secrets["tracking"]:
                        ...
                        st.sidebar.warning("⚠️ Keine Google Sheets ID gefunden")
                        sheet_id = None
                ...
                if sheet_id:
                    ...
                    if service:
                        ...
                        result = (
                            service.spreadsheets()
                            .values()
                            .get(spreadsheetId=sheet_id, range="PREDICTIONS!A:Q")
                            .execute()
                        )
                        ...
                        for i, row in enumerate(values):
                            ...
                            if len(row) > 16 and row[16] == "PENDING":
                                ...
                                predicted_score = row[3] if len(row) > 3 else ""
                                risk_score = row[13] if len(row) > 13 else "N/A"
                        ...
                        if pending_matches:
                            ...
                            if selected_match:
                                ...
                                with st.sidebar.expander(
                                    ...
                                    st.caption(f"**Gesamt:** {goals_total} Tore")
        ...
        with col_s2:
            ...
            if st.session_state.risk_management["stake_history"]:
                ...
                st.metric("Letzte 10 Wetten", "€0.00", delta="0.0%")

        if st.button(
            "🔄 Bankroll auf €1.000 zurücksetzen",
            use_container_width=True,
            key="reset_bankroll",
            ...
            with st.sidebar.expander("📋 Letzte Wetten", expanded=False):
                for i, wette in enumerate(
                    ...
                    with col2:
                        ...
                        color = (
                            "green" if profit > 0 else "red" if profit < 0 else "gray"
                        )
                        st.caption(
                            f"€{profit:+.2f}",
                            help=f"Einsatz: €{wette.get('stake', 0):.2f}",
                        )
                ...
                try:
                    ...
                    if service:
                        result = (
                            service.spreadsheets()
                            .values()
                            .get(spreadsheetId=spreadsheet_id, range="PREDICTIONS!A:A")
                            .execute()
                        )
                        row_count = len(result.get("values", [])) - 1  # Minus Header
        ...
        with col_qa1:
            if st.button(
                "🔄 Cache leeren", key="clear_cache", use_container_width=True
            ):
                st.cache_data.clear()
        ...
        with col_qa2:
            if st.button("📊 ML neu laden", key="reload_ml", use_container_width=True):
                st.session_state.position_ml_model = None
                st.session_state.extended_ml_model = None
...
def add_historical_match_ui():
    ...
    with st.expander("➕ Neues historisches Match", expanded=True):
        ...
        with col_pred2:
            ...
            actual_mu_home = st.number_input(
                "Tatsächliches μ Heim",
                min_value=0.0,
                max_value=5.0,
                value=float(actual_home_goals),
                step=0.1,
                key="actual_mu_home",
            )
            actual_mu_away = st.number_input(
                "Tatsächliches μ Auswärts",
                min_value=0.0,
                max_value=5.0,
                value=float(actual_away_goals),
                step=0.1,
                key="actual_mu_away",
            )
    ...
    if historical_matches:
        ...
        if len(historical_matches) > 20:
            st.caption(f"... und {len(historical_matches) - 20} weitere Matches")
    else:
...
def main():
    ...
    with tab1:
        ...
        available_in_month = {
            d.day for d in available_dates if d.year == m.year and d.month == m.month
        }
        ...
        for week in range(weeks_to_show):
            ...
            for i in range(7):
                ...
                if day_num in available_in_month:
                    ...
                    if cols[i].button(
                        label,
                        key=f"cal_{m.year}_{m.month}_{day_num}",
                        use_container_width=True,
                    ):
                        st.session_state.selected_day = (
        ...
        if sheet_url:
            ...
            if worksheets:
                ...
                # Ergebnis + Demo-Wettauswahl (stabil über Reruns)
                if (
                    "current_result" in st.session_state
                    and "current_match" in st.session_state
                ):
                    # Zeige Demo-Wettauswahl HIER (bleibt nach Rerun sichtbar)
                    if st.session_state.get("enable_demo_mode", False):
    ...
    with tab4:
        ...
        if historical_matches:
            ...
            fig_position_correction = go.Figure(
                data=go.Scatter(
                    x=home_positions,
                    y=home_corrections,
                    mode="markers",
                    ...
                    text=[
                        f"Heim: {m['home_team'].name}<br>Pos: {m['home_team'].position}<br>Korrektur: {m.get('home_correction', 1.0):.3f}"
                        for m in historical_matches
                    ],
                    hoverinfo="text",
                )
    ...
    try:
        ...
        for i, row in enumerate(values):
            ...
            if len(row) > status_col and row[status_col] == "COMPLETED":
                match_info = {
                    "timestamp": (
                        row[col_indices.get("Timestamp", 0)]
                        if len(row) > col_indices.get("Timestamp", 0)
                        else ""
                    ),
                    "match": (
                        row[col_indices.get("Match", 2)]
                        if len(row) > col_indices.get("Match", 2)
                        else ""
                    ),
                    "predicted_score": (
                        row[col_indices.get("Predicted_Score", 3)]
                        if len(row) > col_indices.get("Predicted_Score", 3)
                        else ""
                    ),
                    "actual_score": (
                        row[col_indices.get("Actual_Score", 17)]
                        if len(row) > col_indices.get("Actual_Score", 17)
                        else ""
                    ),
                    "mu_total": (
                        float(row[col_indices.get("μ_Total", 15)])
                        if len(row) > col_indices.get("μ_Total", 15)
                        and row[col_indices.get("μ_Total", 15)]
                        else 0.0
                    ),
                }
...
def check_historical_sheet():
    ...
    try:
        ...
        if "HISTORICAL_DATA" in sheet_names:
            ...
            st.info(
                f"📊 Enthält {row_count - 1 if row_count > 0 else 0} Datensätze (mit Header)"
            )
            ...
            st.info(
                "Das Sheet wird automatisch erstellt, wenn du das erste historische Match speicherst."
            )
